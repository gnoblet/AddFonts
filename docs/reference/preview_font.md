...
- variants:
  ...
  (`list | NULL`) For file-based providers (`provider = "file"`,
  `provider = "url"`, or a `FontProviderFile` object). Named list of
  symbolic variant keys to file paths / URLs / filename stems. Passed
  through to
  [`add_font()`](http://guillaume-noblet.com/AddFonts/reference/add_font.md).
