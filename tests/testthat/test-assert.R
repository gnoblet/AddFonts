...
test_that("assert_null_or_non_empty_character_vector behaves correctly", {
  ...
  # NULL disallowed
  expect_error(
    assert_null_or_non_empty_character_vector(NULL, allow_null = FALSE),
    "NULL is not allowed"
  )
  ...
  # Non-character
  expect_error(
    assert_null_or_non_empty_character_vector(1L),
    "non-empty character vector"
  )

  # Empty character vector
  expect_error(
    assert_null_or_non_empty_character_vector(character(0)),
    "non-empty character vector"
  )
  ...
  expect_error(
    assert_null_or_non_empty_character_vector(c("a", "")),
    "non-empty character vector"
  )

  # All-NA vector
  expect_error(
    assert_null_or_non_empty_character_vector(NA_character_),
    "non-empty character vector"
  )
})
...
test_that("assert_list_of_1_length_character_strings behaves correctly", {
  # Not a list
  expect_error(
    assert_list_of_1_length_character_strings("not a list"),
    "must be a list"
  )
  ...
  expect_error(
    assert_list_of_1_length_character_strings(list("a", 42L)),
    "non-empty character string"
  )

  # Element is empty string
  expect_error(
    assert_list_of_1_length_character_strings(list("a", "")),
    "non-empty character string"
  )

  # Element is NA
  expect_error(
    assert_list_of_1_length_character_strings(list("a", NA_character_)),
    "non-empty character string"
  )

  # Element is length-2 character vector (not a scalar)
  expect_error(
    assert_list_of_1_length_character_strings(list(c("a", "b"))),
    "non-empty character string"
  )
})
...
test_that("assert_pattern_with_ext validates paths and extensions", {
  ...
  expect_error(
    assert_pattern_with_ext("bad file.ttf", ext = ".ttf"),
    "whitespace"
  )

  # Extension mismatch
  expect_error(
    assert_pattern_with_ext("roboto.woff2", ext = ".ttf"),
    "extension is '.ttf'"
  )
  ...
  # Non-logical flag
  expect_error(
    assert_pattern_with_ext("file.ttf", allow_lowercase = "yes"),
    "single logical"
  )
})
